...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb:
        ...
        if settings["button"]:
            ...
            for file in files:
                cap += f"<blockquote><b>📙<a href='https://telegram.me/{temp.U_NAME}?start=files_{file.file_id}'>❗{get_size(file.file_size)}❗{' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file.file_name.split()))}\n\n</a></b></blockquote>"
