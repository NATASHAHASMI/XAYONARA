...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb:
        ...
        if not settings["button"]:
            cap+="<b>\n\n<u>🍿 𝒀𝑶𝑼𝑹 𝑭𝑰𝑳𝑬𝑺 ☟</u></b>\n"
            for file in files:
                cap += f"<b>\n📗<a href='https://telegram.me/{temp.U_NAME}?start=files_{file.file_id}'>𓊈{get_size(file.file_size)}𓊉{' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file.file_name.split()))}\n</a></b>"
    else:
        if settings["button"]:
            ...
            for file in files:
                cap += f"<b>📗<a href='https://telegram.me/{temp.U_NAME}?start=files_{file.file_id}'>𓊈{get_size(file.file_size)}𓊉{' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file.file_name.split()))}\n\n</a></b>"
