    ...
    if settings['button']:
        ...
        btn.insert(0, 
            [
                ...
                InlineKeyboardButton("! Qᴜᴀʟɪᴛʏ !",  callback_data=f"quality#{key}"),
                InlineKeyboardButton("Sᴇᴀsᴏɴs",  callback_data=f"seasons#{key}")
            ]
        )
        btn.insert(0, [
            InlineKeyboardButton("Yᴇᴀʀ",  callback_data=f"year#{key}"),
...
async def seasons_cb_handler(client: Client, query: CallbackQuery):
    ...
    for i in range(0, len(SEASONS)-1, 2):
        btn.append([
            InlineKeyboardButton(
                text=SEASONS[i].title(),
                callback_data=f"fs#{YEAR[i].lower()}#{key}"
            ),
            InlineKeyboardButton(
                text=SEASONS[i+1].title(),
                callback_data=f"fs#{YEAR[i+1].lower()}#{key}"
            ),
        ])
    ...
    btn.insert(
        ...
        [
            InlineKeyboardButton(
                text="👇 ꜱᴇʟᴇᴄᴛ ʏᴏᴜʀ ᴍᴏᴠɪᴇ Yᴇᴀʀ 👇", callback_data="ident"
            )
        ],
    )
    req = query.from_user.id
...
async def languages_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.insert(
        ...
        [
            InlineKeyboardButton(
                text="👇 𝚂𝙴𝙻𝙴𝙲𝚃 𝚈𝙾𝚄𝚁 𝙻𝙰𝙽𝙶𝚄𝙰𝙶𝙴𝚂 👇", callback_data="ident"
            )
        ],
    )
    req = query.from_user.id
...
async def seasons_cb_handler(client: Client, query: CallbackQuery):
    ...
    for i in range(0, len(SEASONS)-1, 2):
        btn.append([
            InlineKeyboardButton(
                text=SEASONS[i].title(),
                callback_data=f"fs#{QUALITY[i].lower()}#{key}"
            ),
            InlineKeyboardButton(
                text=SEASONS[i+1].title(),
                callback_data=f"fs#{QUALITY[i+1].lower()}#{key}"
            ),
        ])
    ...
    btn.insert(
        ...
        [
            InlineKeyboardButton(
                text="👇 ꜱᴇʟᴇᴄᴛ ʏᴏᴜʀ ᴍᴏᴠɪᴇ Qᴜᴀʟɪᴛʏ 👇", callback_data="ident"
            )
        ],
    )
    req = query.from_user.id
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        try:
            ...
            xo = await query.message.reply_sticker(sticker='CAACAgUAAxkBAAEKaC1lFXJerMGLmnNnzkU6p_EMOKHdZgACkA0AAoJzqVTV-B6xYi0PsjAE')
...
async def auto_filter(client, msg, spoll=False):
    ...
    if settings["button"]:
        ...
        btn.insert(0, [
            InlineKeyboardButton("ʏᴇᴀʀ", callback_data=f"year#{key}"),
            InlineKeyboardButton("𝐒𝐞𝐧𝐝 𝐀𝐥𝐥", callback_data=f"sendfiles#{key}")
        ])
    else:
        btn = []
    ...
    if imdb:
        ...
        if not settings["button"]:
            ...
            for file in files:
                cap += f"<b>\n📁 <a href='https://telegram.me/{temp.U_NAME}?start=files_{file.file_id}'>🀄️{get_size(file.file_size)}🎴{' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file.file_name.split()))}\n</a></b>"
    else:
        if settings["button"]:
