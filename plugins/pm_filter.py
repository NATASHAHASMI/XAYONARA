...
from info import ADMINS, AUTH_CHANNEL, AUTH_USERS, SUPPORT_CHAT_ID, CUSTOM_FILE_CAPTION, MSG_ALRT, PICS, AUTH_GROUPS, P_TTI_SHOW_OFF, GRP_LNK, CHNL_LNK, NOR_IMG, LOG_CHANNEL, SPELL_IMG, MAX_B_TN, IMDB, \
    SINGLE_BUTTON, SPELL_CHECK_REPLY, IMDB_TEMPLATE, NO_RESULTS_MSG, TUTORIAL, REQST_CHANNEL, IS_TUTORIAL, LANGUAGES, SEASONS, SUPPORT_CHAT, PREMIUM_USER, EPISODES
from info import *
from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton, CallbackQuery, InputMediaPhoto
from pyrogram import Client, filters, enums
from pyrogram.errors import UserIsBlocked, MessageNotModified, PeerIdInvalid, ChatAdminRequired
from utils import get_size, is_req_subscribed, get_poster, temp, get_settings, save_group_settings, get_shortlink, get_tutorial, send_all, get_cap
    ...
    for i in range(0, len(EPISODES)-1, 5):
        btn.append([
            InlineKeyboardButton(
                ...
                callback_data=f"episode_search#{EPISODES[i].lower()}#{key}#0#{offset}#{req}"
            ),
            InlineKeyboardButton(
                text=EPISODES[i+1].title(),
                callback_data=f"episode_search#{EPISODES[i+1].lower()}#{key}#0#{offset}#{req}"
            ),
            InlineKeyboardButton(
                text=EPISODES[i+2].title(),
                callback_data=f"episode_search#{EPISODES[i+2].lower()}#{key}#0#{offset}#{req}"
            ),
        ])

    btn.append([InlineKeyboardButton(text="⪻ ʙᴀᴄᴋ ᴛᴏ ᴍᴀɪɴ ᴘᴀɢᴇ", callback_data=f"next_{req}_{key}_{offset}")])
    await query.message.edit_text("<b>ɪɴ ᴡʜɪᴄʜ ᴇᴘɪꜱᴏᴅᴇ ᴅᴏ ʏᴏᴜ ᴡᴀɴᴛ, ᴄʜᴏᴏsᴇ ғʀᴏᴍ ʜᴇʀᴇ ↓↓</b>", reply_markup=InlineKeyboardMarkup(btn))
    return

@Client.on_callback_query(filters.regex(r"^episode_search#"))
async def episode_search(client: Client, query: CallbackQuery):
    _, episode, key, offset, orginal_offset, req = query.data.split("#")
    epis = int(episode.split(' ' , 1)[1])
    if epis < 10:
        epis = f'E0{epis}'
    else:
        epis = f'E{epis}'
    ...
    files, n_offset, total = await get_search_results(f"{search} {epis}", max_results=int(MAX_BTN), offset=offset)
    files2, n_offset2, total2 = await get_search_results(f"{search} {episode}", max_results=int(MAX_BTN), offset=offset)
    total += total2
    try:
        n_offset = int(n_offset)
    except:
        try: 
            ...
            n_offset = 0
    files = [file for file in files if re.search(epis, file.file_name, re.IGNORECASE)]
    
    if not files:
        files = [file for file in files2 if re.search(episode, file.file_name, re.IGNORECASE)]
        if not files:
            await query.answer(f"sᴏʀʀʏ {episode.title()} ɴᴏᴛ ғᴏᴜɴᴅ ғᴏʀ {search}", show_alert=1)
            return
    ...
    if n_offset== '':
        ...
        btn.append(
            [InlineKeyboardButton("⪻ ʙᴀᴄᴋ", callback_data=f"episode_search#{episode}#{key}#{offset- int(MAX_BTN)}#{orginal_offset}#{req}"),
             InlineKeyboardButton(f"{math.ceil(offset / int(MAX_BTN)) + 1}/{math.ceil(total / int(MAX_BTN))}", callback_data="pages",),
            ])
    elif offset==0:
        btn.append(
            [InlineKeyboardButton(f"{math.ceil(offset / int(MAX_BTN)) + 1}/{math.ceil(total / int(MAX_BTN))}",callback_data="pages",),
             InlineKeyboardButton("ɴᴇxᴛ ⪼", callback_data=f"episode_search#{episode}#{key}#{n_offset}#{orginal_offset}#{req}"),])
    else:
        btn.append(
            [InlineKeyboardButton("⪻ ʙᴀᴄᴋ", callback_data=f"episode_search#{episode}#{key}#{offset- int(MAX_BTN)}#{orginal_offset}#{req}"),
             InlineKeyboardButton(f"{math.ceil(offset / int(MAX_BTN)) + 1}/{math.ceil(total / int(MAX_BTN))}", callback_data="pages",),
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        btn2 = [[
                 ...
                 InlineKeyboardButton("Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ Lɪɴᴋ", url="https://t.me/MYFLiiX_2")
               ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
    ...
    # don't change anything without contacting me @creatorrio
        ...
        buttons = [[
            InlineKeyboardButton('ᴜꜱᴇʀꜱ ❤️‍🔥', callback_data='user_command'),
            InlineKeyboardButton('ᴀᴅᴍɪɴꜱ ⚡️', callback_data='admin_command')
        ],[
        ...
        buttons = [[
            InlineKeyboardButton('⟸ Bᴀᴄᴋ', callback_data='user_command'),
        ...
        await query.message.edit_text(
            ...
            InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ 📞", url="https://telegram.me/xayoonara")
            ]]
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
            ...
            reply_markup = InlineKeyboardMarkup(btn)
            await query.message.edit_text(
            text=script.DISCLAIMER_TXT.format(query.from_user.mention, temp.U_NAME, temp.B_NAME),
            reply_markup=reply_markup,
            parse_mode=enums.ParseMode.HTML
            )
...
async def auto_filter(client, msg, spoll=False):
    ...
    if not spoll:
        ...
        if len(message.text) < 100:
            search = message.text
            m=await message.reply_text(f"<b><blockquote>𝑺𝒆𝒂𝒓𝒄𝒉𝒊𝒏𝒈 𝑭𝒐𝒓 🔍</blockquote></b> `{search}` ")
            search = search.lower()
            find = search.split(" ")
            search = ""
    ...
    if imdb and imdb.get('poster'):
        try:
            ...
            try:
               if settings['auto_delete']:
                    await asyncio.sleep(300)
                    m=await message.reply_text("<b><i>Don't Send Me Any Files, Document , Pics 😔 Only Movies & Series Name.😉</i></b>")
                    await hmm.delete()
                    await message.delete()
            except KeyError:
                await save_group_settings(message.chat.id, 'auto_delete', True)
