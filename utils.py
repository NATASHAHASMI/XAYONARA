...
async def get_cap(settings, remaining_seconds, files, query, total_results, search):
    ...
    if settings["imdb"]:
        ...
        if IMDB_CAP:
            ...
            cap+="<b>\n\n<u>🍿 𝒀𝑶𝑼𝑹 𝑭𝑰𝑳𝑬𝑺 ☟</u></b>\n\n"
            for file in files:
                cap += f"<b><i>📗<a href='https://telegram.me/{temp.U_NAME}?start=files_{file.file_id}'>𓊈{get_size(file.file_size)}𓊉{' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file.file_name.split()))}\n\n</a></b></i>"
        else:
